...
export default function Auth () {
    ...
    const submitKey = () => {
        if (auth_key){
            fetch([
                'https://api.svdgod.ru/query?action=control_auth_request',
                'auth_key='+auth_key
            ...
            .then(data => {
                ...
                if (data.is_authed === false) {
                    fetch('https://api.svdgod.ru/query?action=control_get_auth_key')
    ...
    useEffect(() => {
        ...
        fetch('https://api.svdgod.ru/query?action=control_is_authed')
